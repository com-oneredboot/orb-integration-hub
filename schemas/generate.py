...
def cleanup_old_files(valid_model_names, valid_enum_names, valid_graphql_names):
    ...
    # Update to new package locations
    py_model_dir = os.path.join(SCRIPT_DIR, '..', 'backend', 'packages', 'orb-models', 'orb_models', 'models')
    py_enum_dir = os.path.join(SCRIPT_DIR, '..', 'backend', 'packages', 'orb-models', 'orb_models', 'enums')
    ...
    # Python models
    if os.path.exists(py_model_dir):
    ...
    # Python enums
    if os.path.exists(py_enum_dir):
        for fname in os.listdir(py_enum_dir):
            if fname.endswith('Enum.py') and fname not in valid_enum_names:
                os.remove(os.path.join(py_enum_dir, fname))
...
def generate_dynamodb_repository(schemas: dict) -> None:
    ...
    """
    output_path = os.path.join(SCRIPT_DIR, '..', 'backend', 'packages', 'orb-models', 'orb_models', 'repository', 'dynamodb_repository.py')
    lines = [
...
def update_orb_models_init_files():
    """Update __init__.py files in orb-models package to import all generated files."""
    logger.info('Updating orb-models __init__.py files')
    
    # Update models __init__.py
    models_dir = os.path.join(SCRIPT_DIR, '..', 'backend', 'packages', 'orb-models', 'orb_models', 'models')
    if os.path.exists(models_dir):
        model_files = [f[:-3] for f in os.listdir(models_dir) if f.endswith('Model.py') and f != '__init__.py']
        model_names = [f[:-5] for f in model_files]  # Remove 'Model' suffix
        
        init_content = '"""\nGenerated model imports.\n\nThis file is auto-generated by generate.py. Do not edit manually.\n"""\n\n'
        
        # Import statements
        for model_file in model_files:
            init_content += f'from .{model_file} import {model_file[:-5]}\n'
        ...
        write_file(os.path.join(models_dir, '__init__.py'), init_content)
        logger.info(f'Updated models __init__.py with {len(model_files)} models')
    
    # Update enums __init__.py
    enums_dir = os.path.join(SCRIPT_DIR, '..', 'backend', 'packages', 'orb-models', 'orb_models', 'enums')
    if os.path.exists(enums_dir):
        enum_files = [f[:-3] for f in os.listdir(enums_dir) if f.endswith('Enum.py') and f != '__init__.py']
        enum_names = [f[:-4] for f in enum_files]  # Remove 'Enum' suffix
        
        init_content = '"""\nGenerated enum imports.\n\nThis file is auto-generated by generate.py. Do not edit manually.\n"""\n\n'
        
        # Import statements
        for enum_file in enum_files:
            enum_class = enum_file[:-4]  # Remove 'Enum' suffix
            init_content += f'from .{enum_file} import {enum_class}\n'
        
        init_content += '\n__all__ = [\n'
        for enum_name in sorted(enum_names):
            init_content += f'    "{enum_name}",\n'
        init_content += ']\n'
        
        write_file(os.path.join(enums_dir, '__init__.py'), init_content)
        logger.info(f'Updated enums __init__.py with {len(enum_files)} enums')
    
    # Update repository __init__.py
    repo_dir = os.path.join(SCRIPT_DIR, '..', 'backend', 'packages', 'orb-models', 'orb_models', 'repository')
    if os.path.exists(repo_dir) and os.path.exists(os.path.join(repo_dir, 'dynamodb.repository.py')):
        init_content = '"""\nRepository utilities.\n\nThis file is auto-generated by generate.py. Do not edit manually.\n"""\n\n'
        init_content += 'from .dynamodb_repository import ENTITY_TABLE_ENV\n\n'
        init_content += '__all__ = [\n    "ENTITY_TABLE_ENV",\n]\n'
        
        write_file(os.path.join(repo_dir, '__init__.py'), init_content)
        logger.info('Updated repository __init__.py')
...
def main():
    ...
    try:
        ...
        # Update orb-models package __init__.py files
        update_orb_models_init_files()
