...
Generated at 2025-08-10T00:38:11.667928+00:00
"""
...
class OwnershipTransferRequests(BaseModel):
    ...
    created_at: float = Field(..., description="Transfer request creation timestamp")

    expires_at: float = Field(..., description="Transfer request expiration timestamp (7 days)")

    updated_at: float = Field(..., description="Last update timestamp")

    completed_at: float = Field(None, description="Transfer completion timestamp")

    failure_reason: str = Field(None, description="Reason for transfer failure or cancellation")

    billing_transition_details: str = Field(None, description="Details of billing transition during transfer")

    fraud_assessment: str = Field(None, description="Fraud detection assessment results")

    notifications_sent: List[str] = Field(None, description="Track notifications sent for this transfer")
