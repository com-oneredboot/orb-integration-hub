...
Generated at 2025-08-10T00:38:11.719664+00:00
"""
...
class NotificationsCreateInput(BaseModel):
    ...
    metadata: str = Field(..., description="Additional context data (applicationId, organizationId, invitationId, etc.)")

    read_at: int = Field(..., description="When the notification was read by the recipient")
    ...
    ttl: str = Field(..., description="TTL timestamp for automatic deletion (set when notification is read)")
...
class NotificationsUpdateInput(BaseModel):
    ...
    metadata: Optional[str] = Field(None, description="Additional context data (applicationId, organizationId, invitationId, etc.)")

    read_at: Optional[int] = Field(None, description="When the notification was read by the recipient")
    ...
    ttl: Optional[str] = Field(None, description="TTL timestamp for automatic deletion (set when notification is read)")
...
class Notifications(BaseModel):
    ...
    sender_user_id: str = Field(None, description="ID of the user who triggered this notification (foreign key to Users)")

    type: str = Field(..., description="Type of notification")

    status: str = Field(..., description="Current status of the notification")

    title: str = Field(..., description="Title/subject of the notification")

    message: str = Field(..., description="Content/body of the notification")

    metadata: str = Field(None, description="Additional context data (applicationId, organizationId, invitationId, etc.)")

    read_at: int = Field(None, description="When the notification was read by the recipient")

    expires_at: int = Field(None, description="When the notification expires (for time-sensitive actions)")

    created_at: int = Field(..., description="When the notification was created")

    updated_at: int = Field(..., description="When the notification was last updated")

    ttl: float = Field(None, description="TTL timestamp for automatic deletion (set when notification is read)")
