...
Generated at 2025-08-10T00:38:11.744989+00:00
"""
...
class PrivacyRequests(BaseModel):
    ...
    request_type: str = Field(..., description="Type of privacy request (DATA_ACCESS, DATA_DELETION, DATA_PORTABILITY, etc.)")

    data_subject_email: str = Field(..., description="Email address of the data subject making the request")

    legal_basis: str = Field(..., description="Legal basis for the privacy request (GDPR Article, CCPA Right, etc.)")

    organization_id: str = Field(None, description="Organization ID if request is scoped to specific organization")
    ...
    access_report: str = Field(None, description="Data access report for GDPR Article 15 requests (JSON)")

    deletion_result: str = Field(None, description="Data deletion results with cryptographic proof (JSON)")

    portable_data: str = Field(None, description="Portable data export for GDPR Article 20 requests (JSON)")
    ...
    compliance_notes: str = Field(None, description="Additional compliance notes or special handling instructions")
