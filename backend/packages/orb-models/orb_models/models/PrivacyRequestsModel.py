...
Generated at 2025-07-25T21:36:32.530067+00:00
"""
...
class PrivacyRequestsUpdateInput(BaseModel):
    ...
    request_id: Optional[str] = Field(
        None, description="Unique identifier for the privacy request (primary key)"
    )
    ...
    data_subject_email: Optional[str] = Field(
        None, description="Email address of the data subject making the request"
    )

    legal_basis: Optional[LegalBasis] = Field(
        None, description="Legal basis for the privacy request (GDPR Article, CCPA Right, etc.)"
    )

    organization_id: Optional[str] = Field(
        None, description="Organization ID if request is scoped to specific organization"
    )

    requester_id: Optional[str] = Field(
        None, description="User ID of the person who submitted the request"
    )

    status: Optional[PrivacyRequestStatus] = Field(
        None, description="Current status of the privacy request"
    )

    received_at: Optional[int] = Field(None, description="When the privacy request was received")

    deadline: Optional[int] = Field(
        None, description="Legal deadline for completing the request (30 days GDPR, 45 days CCPA)"
    )
    ...
    automated_processing: Optional[bool] = Field(
        None, description="Whether the request is being processed automatically"
    )

    access_report: Optional[str] = Field(
        None, description="Data access report for GDPR Article 15 requests (JSON)"
    )

    deletion_result: Optional[str] = Field(
        None, description="Data deletion results with cryptographic proof (JSON)"
    )

    portable_data: Optional[str] = Field(
    ...
    created_at: Optional[int] = Field(
        None, description="When the privacy request record was created"
    )
...
class PrivacyRequests(BaseModel):
    ...
    completed_at: int = Field(None, description="When the privacy request was completed")

    estimated_completion: int = Field(None, description="Estimated completion time for the request")
    ...
    def from_dto(cls, dto: dict) -> "PrivacyRequests":
        return cls(
            ...
            legal_basis=(
                LegalBasis[dto.get("legal_basis", "LegalBasis.UNKNOWN")]
                if dto.get("legal_basis")
                else LegalBasis.UNKNOWN
            ),
            organization_id=dto.get("organization_id"),
            requester_id=dto.get("requester_id"),
            status=(
                PrivacyRequestStatus[dto.get("status", "PrivacyRequestStatus.UNKNOWN")]
                if dto.get("status")
                else PrivacyRequestStatus.UNKNOWN
            ),
