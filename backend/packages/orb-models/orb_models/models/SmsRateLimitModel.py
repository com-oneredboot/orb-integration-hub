...
Generated at 2025-07-25T21:36:32.582017+00:00
"""
...
class SmsRateLimitUpdateInput(BaseModel):
    ...
    phone_number: Optional[str] = Field(
        None, description="Phone number used as the rate limit key (primary key)"
    )

    request_count: Optional[str] = Field(
        None, description="Number of SMS requests made within the rate limit window"
    )

    first_request_time: Optional[int] = Field(
        None, description="Timestamp of the first request in the current rate limit window"
    )

    ttl: Optional[str] = Field(
        None, description="Time-to-live timestamp for automatic record cleanup"
    )
