...
Generated at 2025-07-25T21:36:32.477800+00:00
"""
...
class OrganizationsUpdateInput(BaseModel):
    ...
    owner_id: Optional[str] = Field(
        None, description="ID of the CUSTOMER user who owns the organization (foreign key to Users)"
    )

    status: Optional[OrganizationStatus] = Field(
        None, description="Current status of the organization"
    )

    created_at: Optional[int] = Field(
        None, description="When the organization was created (on payment completion)"
    )
...
class Organizations(BaseModel):
    ...
    status: str = Field(..., description="Current status of the organization")
    ...
    def from_dto(cls, dto: dict) -> "Organizations":
        return cls(
            ...
            status=(
                OrganizationStatus[dto.get("status", "OrganizationStatus.UNKNOWN")]
                if dto.get("status")
                else OrganizationStatus.UNKNOWN
            ),
