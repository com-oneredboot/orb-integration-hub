# file: backend/src/layers/authentication_dynamodb/cognito_groups.py
# author: Claude Code
# created: 2025-06-19
...
class CognitoGroupManager:
    ...
    def remove_user_from_group(self, username: str, group_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Unexpected error removing user {username} from group {group_name}: {str(e)}")
            return False
    ...
    def sync_user_groups(self, username: str, target_groups: List[str]) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(f"Successfully synced groups for user {username}. Target: {target_groups}")
            else:
...
def get_cognito_group_manager(user_pool_id: Optional[str] = None) -> CognitoGroupManager:
    """
    ...
    if not user_pool_id:
        raise ValueError("user_pool_id must be provided or COGNITO_USER_POOL_ID environment variable must be set")
...
def validate_user_access(username: str, required_groups: List[str], user_pool_id: Optional[str] = None) -> bool:
    """
