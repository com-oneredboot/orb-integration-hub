...
class RoleBase:
    ...
    # Instance variables with type hints and default values    role_id: str = field(
        default='',
        metadata={
            "description": "",
            "required": true,
...
class RolePydantic(BaseModel):
    ...
    # Add fields with validation    role_id: str = Field(
        default='',
        description="",
required=True    )    user_id: str = Field(
        default='',
        description="",
required=True    )    application_id: str = Field(
        default='',
        description="",
required=True    )    role_name: str = Field(
        default='',
        description="",
required=True    )    role_type: str = Field(
        default='',
        description="",
required=True    )    permissions: List[] = Field(
        default=list,
        description="",
...
required=True    )    updated_at: timestamp = Field(
        default=None,
        description="ISO 8601 formatted timestamp (e.g., 2025-03-07T16:23:17.488Z)",
required=True    )    active: bool = Field(
        default=False,
        description="",
required=True    )
@dataclass
