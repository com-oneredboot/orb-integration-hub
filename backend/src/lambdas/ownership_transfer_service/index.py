# file: backend/src/lambdas/ownership_transfer_service/index.py
# author: AI Assistant
# created: 2025-06-23
...
from context_middleware import requires_organization_owner, organization_context_required
...
class PaymentStatusService:
    ...
    def get_customer_payment_status(self, user_id: str) -> PaymentStatus:
        ...
        try:
            ...
            if (stripe_status == PaymentStatus.PAYING_CUSTOMER or 
                paypal_status == PaymentStatus.PAYING_CUSTOMER):
                return PaymentStatus.PAYING_CUSTOMER
            elif (stripe_status == PaymentStatus.PAYMENT_METHOD_ON_FILE or 
                  paypal_status == PaymentStatus.PAYMENT_METHOD_ON_FILE):
    ...
    def _get_stripe_customer_status(self, user_id: str) -> PaymentStatus:
        ...
        try:
            response = self.stripe_customers_table.get_item(
                Key={'userId': user_id}
            )
    ...
    def _get_paypal_customer_status(self, user_id: str) -> PaymentStatus:
        ...
        try:
            response = self.paypal_customers_table.get_item(
                Key={'userId': user_id}
            )
...
class BillingRequirementsService:
    ...
    def _get_plan_details(self, plan_level: str) -> Dict[str, Any]:
        ...
        plans = {
            ...
            'ENTERPRISE': {
                'monthly_cost': 299,
                'features': ['enterprise_analytics', 'unlimited_collaboration', 'dedicated_support', 'sso'],
...
class OwnershipTransferResolver:
    ...
    def initiate_ownership_transfer(self, event: Dict[str, Any], org_context) -> Dict[str, Any]:
        ...
        try:
            ...
            billing_requirements = self.billing_service.get_organization_billing_requirements(organization_id)
            ...
            if payment_status == PaymentStatus.PAYING_CUSTOMER:
                ...
                return self._create_payment_validation_request(
                    current_owner_id, new_owner_id, organization_id, 
                    billing_requirements, fraud_assessment
                )
    ...
    def cancel_ownership_transfer(self, event: Dict[str, Any], org_context) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.critical(f"AUDIT_LOGGING_FAILURE for transfer cancellation {transfer_request['transferId']}: {str(audit_error)}")
    ...
    def get_ownership_transfer(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            is_participant = (
                user_id == transfer_data['currentOwnerId'] or 
                user_id == transfer_data['newOwnerId']
            )
        ...
        try:
            ...
            if is_platform_admin:
                ...
                current_owner_response = self.transfer_requests_table.query(
                    ...
                    KeyConditionExpression=boto3.dynamodb.conditions.Key('currentOwnerId').eq(user_id)
                )
                ...
                # Combine results
                all_transfers = (
                    current_owner_response.get('Items', []) + 
    ...
    def _validate_payment_setup(
        ...
        try:
            ...
            return bool(payment_data.get('paymentMethodId') or payment_data.get('billingAgreementId'))
    ...
    def _execute_validated_transfer(self, transfer_request: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Critical: Audit logging failure should be logged but not fail the operation
                logger.critical(f"AUDIT_LOGGING_FAILURE for ownership transfer {transfer_request['transferId']}: {str(audit_error)}")
    ...
    def _get_pending_transfer(self, org_id: str) -> Optional[Dict]:
        ...
        try:
            response = self.transfer_requests_table.scan(
                FilterExpression=boto3.dynamodb.conditions.Attr('organizationId').eq(org_id) &
                               boto3.dynamodb.conditions.Attr('status').eq(TransferStatus.PAYMENT_VALIDATION_REQUIRED.value)
            )
    ...
    def _send_payment_validation_notifications(
        ...
        try:
            ...
            current_owner_notification = {
                ...
                'message': f'You have requested to transfer ownership of your organization. The new owner must complete billing setup before the transfer can be completed.',
                'status': 'UNREAD',
            ...
            new_owner_notification = {
                ...
                'message': f'You have been offered ownership of an organization. Complete billing setup to accept ownership.',
                'status': 'UNREAD',
                'actionRequired': True,
                'metadata': {
                    'transferId': transfer_record['transferId'],
            ...
            logger.info(f"Payment validation notifications sent for transfer {transfer_record['transferId']}")
    ...
    def _send_immediate_transfer_notifications(self, transfer_record: Dict):
        ...
        try:
            ...
            logger.info(f"Immediate transfer notifications sent for transfer {transfer_record['transferId']}")
    ...
    def _send_cancellation_notifications(self, transfer_request: Dict):
        ...
        try:
            ...
            logger.info(f"Cancellation notification sent for transfer {transfer_request['transferId']}")
