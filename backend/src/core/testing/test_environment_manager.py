...
from ..models.dynamodb.repository import DynamoDBRepository
...
class TestEnvironmentManager:
    ...
    def _create_test_tables(self):
        ...
        for table_name, table_config in table_definitions.items():
            try:
                table = self.dynamodb_resource.create_table(**table_config)
    ...
    def seed_basic_environment(self) -> Dict[str, Any]:
        ...
        basic_org = self.factory.create_test_organization(
            name=f"BasicTest_{self.factory.test_session_id}",
            size="small"
        )
        ...
        for scenario_name, scenario_data in scenarios.items():
            if scenario_name == "basic":
                ...
                for test_name, org_data in scenario_data.items():
                    self._seed_organization_data_to_dynamodb(org_data, f"{scenario_name}_{test_name}")
