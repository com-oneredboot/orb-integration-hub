...
export class RateLimitingService {
  ...
  public recordAttempt(
    ...
    if (success) {
      ...
      console.debug(...sanitizeLogArgs([`[RateLimitingService] Successful ${attemptType} for ${identifier.substring(0, 5) + '***'} - rate limit reset`]));
      return;
    }
    ...
    if (state.totalFailedAttempts >= config.maxAttempts) {
      ...
      console.warn(...sanitizeLogArgs([`[RateLimitingService] Account lockout triggered for ${identifier.substring(0, 5) + '***'} (${attemptType})`]));
    } else {
    ...
    console.debug(...sanitizeLogArgs([`[RateLimitingService] Failed ${attemptType} for ${identifier.substring(0, 5) + '***'}`, {
