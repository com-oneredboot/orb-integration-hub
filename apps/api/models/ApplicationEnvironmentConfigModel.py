...
class ApplicationEnvironmentConfig(BaseModel):
    ...
    rate_limit_per_minute: int = Field(..., description="Maximum API requests per minute (default 60)")
    rate_limit_per_day: int = Field(..., description="Maximum API requests per day (default 10000)")
    ...
    webhook_max_retries: Optional[int] = Field(None, description="Maximum retry attempts for failed webhook delivery (default 3)")
    webhook_retry_delay_seconds: Optional[int] = Field(None, description="Delay between retry attempts in seconds (default 60)")
