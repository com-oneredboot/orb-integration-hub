# file: apps/api/lambdas/organizations_resolver/index.py
# author: AI Assistant
# created: 2025-06-22
...
class OrganizationsResolver:
    ...
    def create_organization(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                "CUSTOMER" in cognito_groups
                and "OWNER" not in cognito_groups
                and "EMPLOYEE" not in cognito_groups
            ):
                existing_orgs = self._get_user_organizations(user_id)
                if len(existing_orgs) >= 1:  # Starter plan limit
                    return self._error_response(
                        "Starter plan allows only 1 organization. Upgrade to create more."
                    )
            ...
            try:
                ...
                logger.info(
                    f"Created KMS key {kms_key_info['keyId']} for organization {organization_id}"
                )
            ...
            try:
                ...
                logger.critical(
                    f"AUDIT_LOGGING_FAILURE for organization creation {organization_id}: {str(audit_error)}"
                )

            logger.info(f"Created organization {organization_id} for user {user_id}")

            return {"statusCode": 200, "body": organization_data}
    ...
    def update_organization(self, event: Dict[str, Any], org_context) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Critical: Audit logging failure should be logged but not fail the operation
                logger.critical(
                    f"AUDIT_LOGGING_FAILURE for organization update {organization_id}: {str(audit_error)}"
                )
    ...
    def delete_organization(self, event: Dict[str, Any], org_context) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"KMS key cleanup error for organization {organization_id}: {str(kms_error)}"
                )
            ...
            try:
                ...
                # Critical: Audit logging failure should be logged but not fail the operation
                logger.critical(
                    f"AUDIT_LOGGING_FAILURE for organization deletion {organization_id}: {str(audit_error)}"
                )
    ...
    def get_user_permissions(self, event: Dict[str, Any], org_context) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"statusCode": 200, "body": permissions_data}
    ...
    def get_organization_roles(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            from rbac_manager import RolePermissionMatrix
