# file: apps/api/lambdas/user_status_calculator/index.py
...
def check_cognito_mfa_status(email: str) -> Dict[str, bool]:
    ...
    try:
        ...
        try:
            ...
            if has_devices:
                for device in devices:
                    logger.debug(
                        f"    Device: {device.get('DeviceKey', 'Unknown')} - Created: {device.get('DeviceCreateDate', 'Unknown')}"
                    )
        ...
        mfa_enabled = (
            has_mfa_options
            or has_mfa_settings
            or has_preferred_mfa
            or has_devices
            or (pool_has_mfa and has_user_mfa_prefs)
        )
        ...
        logger.info(
            f"Cognito MFA status for {email}: enabled={mfa_enabled}, complete={mfa_setup_complete}"
        )
        logger.debug(
            f"  Indicators: mfa_options={has_mfa_options}, mfa_settings={has_mfa_settings}, preferred={has_preferred_mfa}, devices={has_devices}, pool_mfa={pool_has_mfa}, user_prefs={has_user_mfa_prefs}"
        )

        result = {"mfaEnabled": mfa_enabled, "mfaSetupComplete": mfa_setup_complete}
...
def calculate_user_status(user_data: Dict[str, Any]) -> str:
    ...
    if email:
        logger.info(
            f"Checking Cognito MFA status for user {user_data.get('userId')} (Cognito is single source of truth)"
        )
        ...
        mfa_status_changed = (
            current_mfa_enabled != cognito_mfa_status["mfaEnabled"]
            or current_mfa_complete != cognito_mfa_status["mfaSetupComplete"]
        )
        ...
        logger.warning(
            f"No email found for user {user_data.get('userId')}, cannot check Cognito MFA"
        )
        mfa_enabled = False
...
def extract_user_data_from_dynamodb_record(
    record: Dict[str, Any],
...
def has_status_relevant_changes(record: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        for field in status_relevant_fields:
            ...
            logger.debug(
                f"Field '{field}': old='{old_value}' new='{new_value}' changed={old_value != new_value}"
            )

            if old_value != new_value:
                changed_fields.append(field)
                logger.info(
                    f"Status-relevant field '{field}' changed from '{old_value}' to '{new_value}'"
                )
...
def lambda_handler(event, _):
    ...
    logger.debug(
        f"Environment variables: USER_POOL_ID={USER_POOL_ID}, USERS_TABLE_NAME={USERS_TABLE_NAME}"
    )
    ...
    try:
        ...
        for record in event.get("Records", []):
            try:
                ...
                logger.debug(
                    f"Extracted user data: userId={user_data.get('userId') if user_data else 'None'}"
                )
                ...
                logger.debug(
                    f"Update check for user {user_id}: status_needs_update={status_needs_update}, mfa_status_updated={mfa_status_updated}"
                )
                ...
                if status_needs_update or mfa_status_updated:
                    ...
                    if status_needs_update:
                        ...
                        logger.info(
                            f"Updating user {user_id} status from {current_status} to {calculated_status}"
                        )
