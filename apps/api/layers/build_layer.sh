...
layer_path="./python/lib/python3.13/site-packages"
...
pipenv install
pipenv run pip freeze > $layer_path/requirements.txt

# Step 3: Fetch the required libraries using the generated requirements.txt
echo "Fetching libraries"
pipenv run pip install -r $layer_path/requirements.txt --target $layer_path
