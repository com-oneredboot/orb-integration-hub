# file: apps/api/layers/organizations_security/context_middleware.py
# author: AI Assistant
# created: 2025-06-23
        ...
        try:
            ...
            if not organization_data:
                raise SecurityViolationError(
                    f"Organization {organization_id} not found or inactive"
                )
            ...
            if not membership_data:
                ...
                if membership_status != "ACTIVE" and not is_platform_admin:
                    raise SecurityViolationError(
                        f"User membership in organization {organization_id} is {membership_status}"
                    )
...
class OrganizationContextMiddleware:
    ...
    def validate_and_inject_context(
        self, event: Dict[str, Any], required_permission: Optional[str] = None
    ) -> OrganizationContext:
        ...
        try:
            ...
            self._log_security_violation(
                user_id=event.get("identity", {}).get("sub"),
                organization_id=(
                    self.extractor.extract_organization_id(event)
    ...
    def _log_security_violation(
        ...
        violation_entry = {
            ...
            "details": details,
            "source_ip": event_context.get("requestContext", {})
            .get("identity", {})
            .get("sourceIp"),
            "user_agent": event_context.get("requestContext", {})
            .get("identity", {})
            .get("userAgent"),
        }
...
def allows_platform_override(func: Callable) -> Callable:
    ...
    def wrapper(self, event: Dict[str, Any], *args, **kwargs):
        ...
        if is_platform_admin:
            ...
            logger.info(
                f"Platform admin override for operation {event.get('info', {}).get('fieldName')}"
            )
