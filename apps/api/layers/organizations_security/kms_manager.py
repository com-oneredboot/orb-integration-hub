# file: apps/api/layers/organizations_security/kms_manager.py
# author: AI Assistant
# created: 2025-06-23
...
class OrganizationKMSManager:
    ...
    def create_organization_kms_key(
        self, organization_id: str, organization_name: str, owner_user_id: str
    ) -> Dict[str, str]:
        """
        ...
        try:
            ...
            response = self.kms_client.create_key(
                ...
                Origin="AWS_KMS",
                Tags=[
                    {"TagKey": "OrganizationId", "TagValue": organization_id},
                    {"TagKey": "OrganizationName", "TagValue": organization_name},
                    {"TagKey": "OwnerUserId", "TagValue": owner_user_id},
                    {"TagKey": "Purpose", "TagValue": "OrganizationEncryption"},
                    {
                        "TagKey": "Environment",
    ...
    def decrypt_organization_data(
        ...
        try:
            ...
            response = self.kms_client.decrypt(
                CiphertextBlob=ciphertext_blob, EncryptionContext=context
            )
    ...
    def delete_organization_key(self, organization_id: str, pending_window_days: int = 30) -> bool:
        ...
        try:
            ...
            self.kms_client.schedule_key_deletion(
                KeyId=key_info["keyId"], PendingWindowInDays=pending_window_days
            )
            ...
            logger.info(
                f"Scheduled deletion of KMS key for organization {organization_id} in {pending_window_days} days"
            )
            return True
                ...
                for alias in page["Aliases"]:
                    if alias["AliasName"].startswith("alias/org-"):
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to get details for key {alias['TargetKeyId']}: {e}"
                            )
